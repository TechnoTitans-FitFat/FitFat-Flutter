...
class ShellfishScreen extends StatelessWidget {
  const ShellfishScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        backgroundColor: context.theme.backgroundColor,
        appBar: const CustomAppBar(title: 'Shellfish'),
        body: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              BlocConsumer<ShellfishCubit, ShellfishState>(
                listener: (context, state) {},
                builder: (context, state) {
                  if (state is ShellfishLoading) {
                    return const Center(
                      child: CircularProgressIndicator(),
                    );
                  } else if (state is ShellfishFailure) {
                    return Center(
                      child: Text(
                        state.errMessage,
                        style:
                  ...
                  } else if (state is ShellfishSuccess) {
